...
public class Player {
    ...
    public void move_player(int dice_value)
    {
        if(curr_pos+dice_value <= 100)
        {
            ...
            new_pos = this_board.snake_ladder.get(new_pos); // check from the snakeladder array if snake or ladder present.

            curr_pos = new_pos;

            TranslateTransition move2 = translate_animation(dice_value);

            SequentialTransition sq = new SequentialTransition(move1, new PauseTransition(Duration.millis(1000)), move2);
